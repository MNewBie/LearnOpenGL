		...
		if (strlen(szOneLine) > 0)
		{
			...
			else if (szOneLine[0] == 'f')
			{
				...
				for (int i = 0; i < 3; i++)
				{
					...
					size_t pos2 = vertexStr.find('/', pos + 1);
...
void Model::Draw()
{
	...
	glMaterialfv(GL_FRONT, GL_AMBIENT, mAmbientMaterial); // 设置材质反射系数
	glMaterialfv(GL_FRONT, GL_DIFFUSE, mDiffuseMaterial); 
	glMaterialfv(GL_FRONT, GL_SPECULAR, mSpecularMaterial); 

	//glMaterialf(GL_FRONT, GL_SHININESS, 64.0f); // 生成一个亮斑， 0 ~128， 数据越大，亮斑越小，聚光度越大；数据越小，亮斑越大，聚光度越小
